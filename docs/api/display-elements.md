...
| `drag_group` | Comma- or semicolon-separated **element ids** on the **same surface** that move with this drag (default: this panel's id only). Use this for **sibling** panels that should translate together but are not parent/child in the layout tree. For `surface:layout(...)` containers, `"auto"` or `drag_group_auto = true` marks the draggable leader as a unit - layout children are parented under it via `parent_id` and follow automatically (do not list descendants manually). Every id in the group must use **`rect.unit = "px"`**. |
...
**Reading the dragged position.** The runtime stores the cumulative canvas-pixel offset for each draggable leader. Use:
