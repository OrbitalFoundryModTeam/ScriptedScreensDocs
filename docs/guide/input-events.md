...
For panel-reposition drag (`props.draggable` on a `panel`), use the callback API instead: **`surface:on_drag(fn)`** at the surface level or **`handle:on("drag_begin", fn)`** / **`handle:on("drag_end", fn)`** at the element level, with **`surface:drag_offset(id)`** to read the stored cumulative position. Use **`surface:is_dragging([id])`** in `tick()` to skip periodic rebuilds that would otherwise snap the panel back to its committed position while the user holds the mouse. See [Programmable Visor HUDs - Dragging visor panels](/guide/visor-huds#dragging-visor-panels) and [Display elements - Draggable panels](/api/display-elements#draggable-panels).
